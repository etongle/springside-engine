...
public class ThriftHandler extends ChannelInboundHandlerAdapter {
	...
	private void writeResponse(ChannelHandlerContext ctx, TNettyTransport transport) {
		ByteBuf response = transport.getFramedOutput();
		ctx.writeAndFlush(response);
	}

	class ProcessorTask implements Runnable {
		ChannelHandlerContext ctx;
		TNettyTransport transport;
		...
		public void run() {
			...
			try {
				...
				writeResponse(ctx, transport);
			} catch (TException e) {
				logger.error("Exception happen when call processor", e);
				// TODO: response wrong exception
			}
		}
	}
}
